...
describe('NotificationPage - 히어로 핸드오프', () => {
  it('복귀(POP) 시 저장된 히어로 사본을 출발 좌표에 되살리고, 원본 썸네일의 exit-key는 감춘다', () => {
    ...
    const suppressedThumbnail = suppressedItem.querySelector(
      'img[src="https://example.com/thumb-7.jpg"]',
    );
    expect(suppressedThumbnail).toHaveClass('opacity-0');
    expect(suppressedThumbnail).not.toHaveClass('transition-opacity');
    // 다른 알림의 썸네일은 영향 없음
